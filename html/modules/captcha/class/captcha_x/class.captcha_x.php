<?php
// $Id: class.captcha_x.php,v 1.1 2012/03/31 16:04:15 ohwada Exp $
...
// 2008-01-26 K.OHWADA
// BUG: not work in safe mode
// add $flag_debug in validate()

// 2007-08-25 K.OHWADA
// BUG 4694: Invalid font filename in PHP 5.2.3

// 2007-05-20 K.OHWADA
// Notice [PHP]: Undefined index: captcha

// 2007-02-18 K.OHWADA
// include once this class
// add $session_name
// add $flag_header in handle_request()
...
//  function handle_request () {
    function handle_request ( $flag_header=true ) 
    {
        extract ( $this->INI);
...
//  function validate ( $user_string ) {
    function validate ( $user_string, $flag_debug=false ) 
    {
        extract ( $this->INI);
...
//      if ( $md5 == $_SESSION['captcha']) {
        if ( isset($_SESSION[ $this->session_name ]) && ($md5 == $_SESSION[ $this->session_name ])) {
        ...
        if ( $flag_debug )
		{
			echo " validate $user_string $md5 <br />\n";
			echo "<pre>";
			print_r( $_SESSION );
			echo "</pre>\n";
		}
...
// output message in safe mode
// putenv() [function.putenv]: Safe Mode warning: Cannot set environment variable
...
//      $_SESSION['captcha'] = $md5;
        $_SESSION[ $this->session_name ] = $md5;
